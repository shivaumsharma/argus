...
st.write(
    ...
    "dashboard, not a simplified marketing version of them. Every stage is tagged to the specific brief "
    "requirement it answers, and every stat is live-computed from this run, not hand-typed."
)
_eval_report = cached_eval_report(version)
_scale_report = cached_scale_stress_report(version)
if _eval_report:
    _arch_path = architecture_diagram.render_architecture_diagram(_eval_report, _scale_report)
    st.iframe(src=_arch_path, height=2020)
else:
    st.info("Run **Re-run detection** in the sidebar to generate this.", icon=":material/info:")
