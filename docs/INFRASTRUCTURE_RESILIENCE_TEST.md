...
- The same 5 injected corruptions: **zero crash**, **5/5 dropped and
  ...
  end-to-end** on the cleaned data (166 candidate clusters, 75 flagged on
  the current `SEED=42668329` dataset — consistent with the frozen
  dataset's own known 75-flagged baseline, since removing 5 malformed rows
...
- **Zero regression on the real frozen dataset**: run against the actual,
  ...
  primary dataset was re-frozen twice afterward for unrelated reasons —
  grounded device-sharing statistics, then a grounded hard-signal ring-size
  range, see `ARCHITECTURE.md` — which is why today's headline numbers
  differ from this specific regression check.)
