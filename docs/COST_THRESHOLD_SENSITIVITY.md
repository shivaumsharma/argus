...
- **False-negative cost (a missed ring) is computed, not assumed.** For every
  ...
  — real money already in the dataset. Result: **Rs 1,041 average per missed
  ring** (range Rs 0-2,189 across 80 rings).
...
| `review_plus_moderate_churn` | Rs 329 | + 5% churn probability on an assumed 5-future-order LTV (grounded in this dataset's real mean order value, Rs 714). |
...
| 1 | 85 | 100.0% | 2.5% | 0 | 1 |
| 2 | 82 | 100.0% | 2.5% | 0 | 1 |
| **3 (current)** | 74 | 91.2% | 2.5% | 7 | 1 |
| 4 | 65 | 80.0% | 2.5% | 16 | 1 |

**Real finding: confounder FPs stay flat at 1 across every threshold value on
this branch.** The one real false positive in the frozen dataset (a "tight"
household) has `shared_device=True`, so it never reaches this branch's
decision at all — this specific sweep has no effect on it either way.
Cost-optimal threshold is therefore always 1 in every FP-cost scenario, but
**not because aggressive flagging is free in general** — only because this
particular dataset's sole known false positive happens to sit on a different
branch. Reported exactly as found, not stretched into a general claim this
data can't support.
...
**Real finding: recall is completely flat (73/80 rings) across all four
...
production default (3), 6 (15%) at threshold 4. **That makes threshold=2 a
strict improvement over the current default on every metric measured here —
same recall, fewer false positives — true regardless of any FP-cost
assumption.**
...
| 2 | 150 | 329 | 1,222 |
| 3 (current) | 7,434 | 7,613 | 8,506 |
| 4 | 16,799 | 16,978 | 17,871 |
| **Device-clear sweep** | | | |
| 1 | 7,284 (optimal) | 7,284 (optimal) | 7,284 (optimal) |
| 2 | 7,284 | 7,284 | 7,284 |
| 3 (current) | 7,434 | 7,613 | 8,506 |
| 4 | 8,184 | 9,256 | 14,614 |

## Honest read

At every FP-cost scenario grounded in this dataset's real numbers, the fraud
recovered per ring (Rs 1,041 average) is large enough relative to a single
...
dev-split tuning pass, since it costs a real, avoidable false positive for no
