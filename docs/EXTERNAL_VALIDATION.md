...
**Result** (114,634 nodes, 133,700 edges, 24,864 labeled in this mirror — 9.8% illicit base rate):

*Denominator note, corrected since this doc was first written:* `classes.csv` labels the full original Elliptic dataset (46,564 labeled, 4,545 of them illicit — Weber et al.'s real total), but this mirror's features/edge list only covers 114,634 of the original 203,769 transactions. 21,700 labeled transactions (2,098 of them illicit) have no node in this mirror's graph at all and are structurally uncapturable by anything running on this data. Every recall figure below is against the correct, restricted denominator — **24,864 labeled, 2,447 illicit** — not the full-dataset one; using the full-dataset count inflates the denominator and understates recall by very close to half.
...
| Stage 3 — Louvain (unchanged) | 261 | 21 | **24.4%** (597/2,447) | **72.0%** | **7.3x** |
...
|---|---|---|---|---|
| 0.5 (reported above) | 21 | 597 | 24.4% | 72.0% (7.3x) |
| 0.4 | 26 | 963 | 39.4% | 58.8% (6.0x) |
| 0.3 | 34 | 1,217 | 49.7% | 51.8% (5.3x) |
| 0.2 | 46 | 1,759 | 71.9% | 38.8% (3.9x) |
| 0.1 | 58 | 2,033 | **83.1%** | 30.8% (3.1x) |

The same clustering already contains **3.4x more identifiable illicit transactions** (2,033 vs. 597) than the reported headline, at a threshold that still clears a real 3.1x lift over base rate — not noise. Precision falls correspondingly (72.0% → 30.8%), a genuine and expected recall/precision tradeoff, not a free improvement — the point is not that 0.1 is the "right" threshold either. It's that **0.5 was never chosen for this dataset; it was inherited**, and the headline 829/597/72.0% is one point on a curve this test never swept, not a discovered ceiling. (Stage 2's connected components show the identical pattern more starkly: 0 flagged at 0.5, but 1,932 illicit captured — 79.0% recall — at threshold 0.1, still concentrated in the smaller, denser components rather than the giant low-density blobs described above.) Reported here as a real, quantified gap in how this test was run, not smoothed over: the 72.0%/7.3x figure is real and defensible as one operating point, but it is not evidence that 597 is the most this unmodified clustering can find on this data.
...
**203 such real structures exist** (≥2 members) in this graph, holding 611 of the 2,447 total labeled-illicit transactions in this mirror — the remaining 1,836 illicit transactions (75.0%) have *no* illicit neighbor at all and are structurally invisible to any connected-structure-based method by definition, the same "zero-shared-attribute" blind spot already named in this project's own primary-system limitations. Of those 203 real structures, detection (Stage 3's 21 flagged communities) found **51 at ≥50% coverage each (25.1%)** — covering 214 of the 611 illicit transactions that sit inside a real structure (35.0%). This is a real, distinct, lower number than the node-level recall reported above (24.4%, 597/2,447) precisely because that node-level figure counts every illicit transaction *reachable at all* through a flagged community, including ones diluted inside much larger structures; this coverage figure asks the stricter, structural question — how many of the real, connected illicit sub-communities did detection substantially isolate as their own group. Reproduced by `python -m backend.external_validation.elliptic`.
...
**Label-blind classifiers, matched to density's own recall — is the density rule's advantage just the ground-truth label itself?** A real logistic regression and a real XGBoost classifier, trained on Elliptic's 166 per-transaction columns (time-step + Weber et al.'s 165 local/aggregated features), never the label as an input — only as the training target and for threshold selection on a held-out split the label was never chosen against. At the same recall the density rule operates at (~18–24%, matched exactly rather than compared at whatever recall each method happens to land on), XGBoost reaches 75.2% precision against the density rule's 79.1% — close enough that the density rule's edge over a real, label-blind classifier is small and does not look like it's simply reading the answer key back. Reproduced by `python -m backend.external_validation.elliptic` (`label_blind_classifier_check`).

**Graph-blind ensemble — does a classifier recover what clustering structurally cannot reach?** 75% of all illicit transactions in this mirror have zero illicit neighbors at all (`structural_coverage_density_detector` above) — not a threshold problem, a structural one: no connected-component or community-detection method, however tuned, can ever flag a node with no fraudulent neighbor. Tested directly: does OR-ing the already-trained XGBoost classifier's own per-transaction flag onto the existing graph-based flag recover any of that population? On the held-out test split, of 442 graph-isolated illicit transactions, graph-alone recovers 70 (15.8%); the ensemble recovers 432 (97.7%). Overall test-split recall rises from 18.0% (graph alone) to 95.2% (ensemble), and **precision rises too** — 79.1% → 93.96% — because the classifier is simply a much stronger per-transaction signal than a Louvain community's mean density. The one real cost sits elsewhere: relative to running the classifier alone (98.6% precision, 94.9% recall), adding the graph flag on top gives a small additional recall gain at a small precision cost (98.6% → 93.96%) — so the honest recommendation on this data shape is *classifier as primary, graph flag as a structural backstop*, not the reverse. This is a genuinely new, real, positive result — not present when this document was first written — reproduced by `python -m backend.external_validation.elliptic` (`graph_blind_ensemble_check`) and surfaced live on the External Validation dashboard page.
...
| Elliptic | Bitcoin transaction flow | 829 (597 illicit) | 24.4% | 72.0% | Yes — sample large enough |
...
**On all three datasets, the 0.5 flag threshold was checked directly and confirmed to not be a discovered ceiling** — every recall figure above rises substantially (17.0%→67.3% YelpChi, 1.1%→25.0% Amazon, 24.4%→83.1% Elliptic) when the same already-computed clusters are re-scored at threshold 0.1, always at a real, expected precision cost. And on Elliptic specifically, a structural check independent of the fraud-label question entirely confirms the flagged groups are genuinely connected real transactions, not a Louvain partitioning artifact (21/21) — and a real-classifier ensemble recovers 97.7% of the illicit transactions that clustering structurally cannot reach at all (see "Graph-blind ensemble" above), the strongest single result in this document. Every number in this document is live-computed by `python -m backend.external_validation.run all`, not hand-copied — the dashboard's External Validation tab reads the identical JSON.
