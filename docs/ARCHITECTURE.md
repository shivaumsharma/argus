...
comparable number: **FRAUDAR exactly recovers 5 of the 40 planted
hard-signal rings (12.5%), against Stage 2's 100% (40/40) on the identical
40 rings from the identical underlying signals** — real cross-validation for
those 5, and a measured illustration of why: generic greedy density-peeling
...
single one of the 40 planted confounders either. (This recall dropped from
37.5% on the prior seed — reported plainly; real household/hostel
device-sharing now grounded in the dataset adds more competing dense
structure for the same single-pass peeling to dilute, a plausible but not
rigorously isolated mechanism — see `FRAUDAR_CROSSCHECK.md`.) The qualification: the rule
...
- **LLM confidence scores are self-reported, not calibrated probabilities — but there's now a real check, not just a caveat.** On the re-frozen `SEED=51238923` dataset, the primary dataset alone produces 3 negative examples (63 correct of 66 LLM-scored clusters — up from 1 of 74 on the prior seed, since 2 confounders and one other cluster are now wrongly flagged; see Known Limitations below), still thin enough to make calibration hard to test on its own. `backend/confidence_calibration.py` was rebuilt to combine that base with a purpose-built supplementary batch run through `custom_scenario.py`'s real pipeline+Stage 8 path (isolated scratch space, same guarantee as every other custom-scenario run): clear rings, clear organic clusters, and a shared-device "tight household" replication matching the real archetype behind the primary dataset's known misses. That batch reproduced the edge case for real (10 of 12 tight-household replications wrongly flagged), giving **13 negative examples across 81 scored clusters** — enough spread to see a genuine trend: 50% accuracy at 0.7–0.8 confidence (n=2), 45.5% at 0.8–0.9 (n=22), 100% at 0.9–1.0 (n=57). Confidence does track accuracy on this combined dataset. See the Metrics page for the full decile breakdown, and the caveat that the supplementary batch deliberately over-samples the known edge case for calibration purposes — it is not a claim about how often this failure mode occurs naturally.
