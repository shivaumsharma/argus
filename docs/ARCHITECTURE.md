...
- **The two eval misses are real, not hidden.** `RING_SOFT_02` (a deliberately "hard mode" soft-signal ring) was correctly clustered by Stage 3 but fell into Stage 5's conservative default-no-flag branch — a genuine gap in the soft-signal thresholds, not a pipeline bug. `CONF_HOUSEHOLD_01` (a deliberately "tight" household confounder) was wrongly flagged because its compressed 9.8-day signup window failed the ≥21-day spread-out check despite otherwise-clear organic signals (order-value CV 0.50, 8.7 sessions/member post-signup) — both are visible and explained in the dashboard's Confounders and Metrics tabs.
