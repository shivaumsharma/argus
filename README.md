...
Synthetic cohort: **7,500 accounts** — 40 hard-signal rings, 40 soft-signal rings, and 40 planted legitimate confounders (120 planted cases, ~1,502 labeled accounts), plus ~5,998 unconnected background accounts as noise. Generated with `SEED=42668329` — re-frozen from `SEED=51238923` specifically to ground the hard-signal ring-size range in real, measured fraud-cluster sizes from YelpChi/Amazon (median size 2, previously an invented minimum of 3 — see [`docs/EXTERNAL_VALIDATION.md`](docs/EXTERNAL_VALIDATION.md#deriving-synthetic-data-realism-from-real-fraud-cluster-structure)), never seen while Stage 5's thresholds or Stage 3's Louvain resolution were being set — `generate -> pipeline -> eval` was run exactly once on this seed and these numbers are reported as-is, not iterated on.

| Metric | Value |
|---|---|
| Hard-signal ring recall | **100%** (40/40) |
| Soft-signal ring recall | **85.0%** (34/40) — the real test of the approach |
| Confounder false-positive rate | **2.5%** (1/40) |
| Cluster-level precision | **98.7%** |

Every miss is individually traceable, not a bug: all 6 missed soft rings are the deliberately "hard mode" variant (slower referral claims, noisier order-value templating); the 1 wrongly-flagged confounder is a "tight" household (a compressed, borderline-organic signup window). Zero misses on the easy cases of either category — see [`data/processed/eval_report.json`](data/processed/eval_report.json) and [Known Limitations](docs/ARCHITECTURE.md#known-limitations-honest) below.
...
**Tested on real, independently-labeled fraud data too, not just our own construction — node-level, not ring-level, and reported with raw counts, not just rates.** The unmodified Stage 2/3 clustering, run against YelpChi (99.2% precision, 6.8x lift over base rate on real fake-review collusion, 1,143 flagged accounts — large enough to trust) and Elliptic (72.0% precision, **7.3x lift** on a real Bitcoin transaction graph, 829 flagged accounts — deliberately the hardest test available, zero device/payment-instrument analog to lean on, read as a generalization proof-of-concept: the floor, not the ceiling, for what the same architecture should do on Razorpay-native signals). Checked directly, not assumed: that test only ever ran Stage 2/3 (clustering) — Stage 4/5's actual behavioral rules never applied at all, substituted by one bare 50%-density threshold inherited unchanged from YelpChi/Amazon. Sweeping that one threshold on the exact same clusters finds **3.4x more identifiable illicit transactions (2,033 vs. 597)**, still at a real 3.1x lift over base rate — confirming 829/597/72.0% is one point on an unswept curve, not a discovered ceiling. Amazon's result doesn't make that list: only 4 clusters / 11 accounts total were ever flagged, so its 82%-looking figure is reported as the raw count it is (9 of 11 correct) rather than a rate — too small a sample to trust either way; its unused third relation type was tested 3 ways and confirmed to add nothing. **The strongest single external-validation result: 75% of Elliptic's illicit transactions have zero fraudulent neighbors at all — structurally unreachable by graph clustering, no matter how tuned — and OR-ing in the per-transaction classifier already built for the label-blind check recovers 97.7% of that population, with precision rising too (79.1% → 93.96%), not falling.** Full methodology, raw counts, the threshold sweep, and the node-vs-ring-level distinction in [`docs/EXTERNAL_VALIDATION.md`](docs/EXTERNAL_VALIDATION.md).

**Fairness audit against RBI FREE-AI's "Fair" pillar**: shared device/IP is also just how families and hostel residents live, not fraud — so does Stage 5's confounder false-positive rate, and separately ring recall, skew by geography? Code-level check confirms `home_pincode` is never read anywhere in Stages 1-5 (no direct bias path); tagging the 40 confounders and 80 rings by a real, 3-tier city classification (Tier-1 metro / Tier-2 city / Tier-3-other, using verified real PIN prefixes — explicitly *not* RBI's own, much broader 100K+ banking definition, a distinction worth getting right rather than conflating) finds 0 FP in Tier-1/Tier-2 vs. 1 FP in Tier-3 confounders, and a ring-recall spread (88.9% Tier-1 vs. 95.1% Tier-3) that traces to a single miss on only 9 Tier-1 rings — honestly reported as **too small a sample to mean anything either way**, not smoothed over just because the raw numbers look uncomfortable at a glance. Full methodology, the RBI-classification clarification, and the "what production would need" gap in [`docs/FAIRNESS_AUDIT.md`](docs/FAIRNESS_AUDIT.md).
