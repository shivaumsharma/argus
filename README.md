...
Synthetic cohort: **7,500 accounts** — 40 hard-signal rings, 40 soft-signal rings, and 40 planted legitimate confounders (120 planted cases, ~1,483 labeled accounts), plus ~6,017 unconnected background accounts as noise. Generated with `SEED=51238923` — re-frozen from the original `SEED=20260828` specifically to turn on `USE_GROUNDED_DEVICE_SHARING` (household/hostel device-sharing probabilities grounded in real Indian survey data — see [Known Limitations](docs/ARCHITECTURE.md#known-limitations-honest)), never seen while Stage 5's thresholds or Stage 3's Louvain resolution were being set — `generate -> pipeline -> eval` was run exactly once on this seed and these numbers are reported as-is, not iterated on.
...
| Cluster-level precision | **96.1%** |

More realistic device-sharing among legitimate households/hostels moved the confounder false-positive rate up (2.5%→5.0%, 1→2 wrongly-flagged confounders) — the honest, predicted consequence of making the detection problem harder, not a regression. Soft-signal recall improved slightly (82.5%→85.0%) on this seed, and hard-signal recall held at 100%.
...
**FRAUDAR cross-check**: an independent, published, camouflage-resistant densest-subgraph method (Hooi et al., KDD 2016 best paper) run standalone against the same frozen dataset's device/instrument/subnet attributes only — no referral timing, no order data, which means this specific check is scoped to hard-signal rings only (soft rings are defined by having no device/instrument signal at all, so they're structurally out of scope here). Algorithm verified against a public reference implementation, not approximated from memory. The one comparable number: **FRAUDAR recovers 5 of the 40 planted hard-signal rings exactly (12.5%), against Stage 2's 100% (40/40) on the identical 40 rings from the identical underlying signals** — real cross-validation for those 5 from a detection mechanism that never sees ground truth, and a concrete, measured illustration of why connected components (extracted whole regardless of relative density) outperforms generic density-peeling (which dilutes smaller rings into a larger residual block) for this specific problem. It also never cleanly flags a single one of the 40 planted confounders. This number dropped substantially (37.5%→12.5%) on the re-frozen dataset — real device-sharing now present in legitimate confounders (per the grounding above) adds competing dense structure FRAUDAR's block-selection has to sort through, exactly the kind of realistic noise a synthetic-only dataset previously lacked. "Independent" carries one honest asterisk though — building this surfaced a case where the stopping rule for how many blocks to report was first tuned using inside knowledge of our own ground truth, caught, and fixed to a dataset-blind threshold. Full writeup, including that story and a real bug found and fixed along the way, in [`docs/FRAUDAR_CROSSCHECK.md`](docs/FRAUDAR_CROSSCHECK.md).
